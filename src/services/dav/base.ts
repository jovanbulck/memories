...
export async function getFiles(photos: IPhoto[], opts?: GetFilesOpts): Promise<IFileInfo[]> {
  ...
  // Remove any photos from shared albums
  ...
  for (const [album, photos] of Object.entries(sharedAlbumPhotos)) {
    cache = cache.concat(cache, getAlbumFileInfos(photos, album.split('/')[0], album.split('/')[1]));
  }
